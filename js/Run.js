export function runTheMindFuck(onLoadEvent) {
	// VARIABLES
	var xnode = new Array();
	var drawtimer = 0;
	var canvas_id = 'main';
	...
	window.onmouseup = function (e) {
		drawtimer = 0;
		for (i = 0; i < xnode.length; i++) xnode[i].implodeTo(mx, my, 10, 500);
	};
	var lmx;
	var lmy;
	var mx = 0;
	...
	function animateAllNodes() {
		...
		for (i = 0; i < xnode.length; i++) xnode[i].explodeFrom(mx, my, 1, 300);
	...
	function removeFurthestNode() {
		...
		for (i = 0; i < xnode.length; i++) {
			var dist = Math.sqrt(
				Math.pow(Math.abs(winW / 2 - xnode[i].x), 2) +
					Math.pow(Math.abs(winH / 2 - xnode[i].y), 2)
			);
			if (dist > furthest) {
				furthest = dist;
				furthest_i = i;
	...
	window.onmousemove(onLoadEvent);
	detectBrowserSize();
	resizeCanvas();
